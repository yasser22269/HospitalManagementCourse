    ...
    public function store($request){
        ...
        try {
            ...
            $this->verifyAndStoreImage(
                $request,'photo','doctors',
                'upload_image',$doctors->id,'App\Models\Doctor');
            ...
            if ($request->has('photo')){
                ...
                if ($doctor->image){
                    ...
                    $this->Delete_attachment(
                        'upload_image','doctors/'.$old_img,$request->id);
                }
                //Upload img
                $this->verifyAndStoreImage(
                    $request,'photo','doctors',
                    'upload_image',$request->id,'App\Models\Doctor');
            }
    ...
    public function destroy($request)
    {
        if($request->page_id==1){
            ...
            if($request->filename){
                ...
                $this->Delete_attachment(
                    'upload_image','doctors/'.$request->filename,
                    $request->id,$request->filename);
            }
